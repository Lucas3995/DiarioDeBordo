...
### 8. Estabelecer Linguagem Ubíqua

Antes de produzir a saída final, alinhar termos do domínio:

- **Listar termos-chave** que apareceram na demanda e no código existente.
- **Verificar consistência:** o mesmo conceito deve ter o mesmo nome em toda a documentação e no código.
- **Registrar na saída:** incluir seção "Glossário de domínio" com termos, definições e observações de uso.
- **Importância:** o maestro e o mercenario usarão esses termos; inconsistências aqui propagam ambiguidade no código.

Ver [reference.md](reference.md) §Linguagem Ubíqua para guia detalhado.

### 9. Identificar dados pessoais e requisitos de proteção (LGPD)

Ao analisar a demanda, verificar se envolve dados pessoais:

- **Dados pessoais:** nome, CPF, email, endereço, telefone, dados biométricos, dados de saúde, etc.
- **Se identificar dados pessoais**, sinalizar no entregável com checklist:
  - [ ] Finalidade do tratamento clara e documentada?
  - [ ] Consentimento do titular necessário?
  - [ ] Dados coletados são os mínimos necessários (minimização)?
  - [ ] Direitos do titular previstos (acesso, correção, exclusão)?
  - [ ] Período de retenção definido?
  - [ ] Anonimização/pseudonimização necessária para analytics?
- **O maestro** lerá essa sinalização e transformará em requisitos técnicos concretos (endpoints, validações, rotinas de expurgo).
- **O quadro-de-recompensas** criará testes que verificam conformidade com esses requisitos.

Ver [reference.md](reference.md) §Proteção de Dados para checklist detalhado.

### 10. Avaliar e mitigar riscos do uso de IA

Quando a demanda envolver geração de conteúdo, automação ou assistência por IA:

- **Identificar áreas de risco:** geração de texto/dados, decisões automatizadas, classificação, recomendação.
- **Aplicar estratégias de mitigação por área:**
  - Revisão humana obrigatória para outputs voltados ao usuário.
  - Validação de dados gerados contra fonte autoritativa.
  - Testes de viés e fairness quando houver classificação de pessoas.
  - Transparência: informar ao usuário quando conteúdo é gerado por IA.
- **Registrar no entregável:** seção "Mitigações IA" com risco identificado + estratégia aplicável.

Ver [reference.md](reference.md) §Mitigação de Riscos ao Usar IA para catálogo de estratégias.
