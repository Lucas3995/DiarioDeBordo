...
### O que o sistema NÃO é e NÃO faz

Para evitar ambiguidades sobre o escopo, o sistema explicitamente **não** é e **não** faz:

- **Não é um player de mídia completo.** O sistema embute reprodução de conteúdo quando tecnicamente viável (embed de vídeo, leitor de texto, player de áudio), mas não pretende substituir players dedicados como VLC, Spotify ou leitores de e-book. A reprodução é uma conveniência, não o propósito primário.
- **Não faz download nem cache de conteúdo online para acesso offline.** O sistema gerencia informações sobre conteúdos (metadados, progresso, anotações), não armazena cópias do conteúdo em si. Se um vídeo do YouTube está offline, o sistema permite gerenciar seu registro mas não possui uma cópia local do vídeo.
- **Não tem componente social.** Não há perfis públicos, compartilhamento entre instâncias, feed de outros usuários, nem comunicação entre usuários de instâncias diferentes. O multi-usuário é local (várias pessoas no mesmo PC com dados independentes).
- **Não faz recomendação de conteúdo novo.** O sistema não sugere conteúdos que o usuário não cadastrou ou não segue. A seção "Descobrir no acervo" (seção 5.1) mostra conteúdos que o próprio usuário já possui mas ainda não consumiu — não é descoberta de conteúdo externo. Recomendação baseada em notas e avaliações do usuário é mencionada no esboço original como possibilidade futura mas não está no escopo atual.
- **Não é uma plataforma de publicação.** O sistema não permite que o usuário publique conteúdo para outros consumirem. É uma ferramenta de gestão pessoal, não de distribuição.
...
#### 4.4.2. Identidade e Deduplicação de Conteúdo

A regra de progresso global pressupõe que um conteúdo é representado por um único registro no sistema. Isso gera uma preocupação de domínio: o mesmo conteúdo pode aparecer em múltiplas subscrições (o mesmo vídeo publicado em dois feeds diferentes, o mesmo artigo referenciado em duas fontes). Se o sistema criar dois registros para o mesmo conteúdo, o progresso deixa de ser global — é duplicado.

**Intenção de design:** Um conteúdo = um registro. Quando o sistema detecta que um item de um feed corresponde a um conteúdo já registrado (por URL de fonte, por título + criador, ou por outro mecanismo de correspondência), ele deve referenciar o registro existente em vez de criar um novo. O mecanismo específico de detecção de duplicatas (heurísticas de correspondência, resolução de conflitos, tratamento de falsos positivos) será definido em fase posterior de design, mas a intenção é declarada aqui como regra de domínio: duplicação de registros para o mesmo conteúdo é indesejada e o sistema deve ativamente tentar evitá-la.

O usuário deve poder, em caso de erro de deduplicação (o sistema juntou dois conteúdos que são de fato diferentes, ou manteve separados dois que são o mesmo), corrigir manualmente — unindo registros ou separando-os (conforme princípio 2.1).

#### 4.4.3. Histórico de Ações
...
- Capacidade de recuperar estados anteriores (apoiado pelo histórico de ações da seção 4.4.3).
...
Caminho no modelo: O usuário cria uma coletânea Guiada "Resident Evil" com ordenação habilitada e acompanhamento sequencial ativo (configurações comportamentais da seção 4.2). Dentro dela, cria coletâneas filhas — "RE — Jogos" (Guiada, com ordenação e acompanhamento sequencial), "RE — Filmes" (idem), "RE — Livros" (idem). Cada jogo, filme ou livro é um conteúdo com formato respectivo (interativo, vídeo, texto). Não há restrição técnica de formato dentro de nenhuma coletânea — a organização por mídia é decisão do usuário, não imposição do sistema. Progresso é rastreado individualmente em cada conteúdo e refletido globalmente.

**Resultado:** O modelo atende completamente este cenário. A composição de coletâneas (coletânea-mãe contendo coletâneas-filhas) substitui a necessidade de tipos distintos como "Série" e "Franquia".
